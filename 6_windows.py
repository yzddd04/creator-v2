...
# --- SETUP DATABASE ---
def try_connect_mongo(max_retries=10, delay=5):
    ...
    for attempt in range(1, max_retries+1):
        try:
            if tried_insecure:
                print("[WARNING] Menggunakan tlsAllowInvalidCertificates=True (kurang aman, hanya untuk testing)")
                cloud_client = MongoClient('mongodb+srv://ahmadyazidarifuddin04:Qwerty12345.@creatorweb.zpsu4ci.mongodb.net/?retryWrites=true&w=majority&appName=creatorWeb', serverSelectionTimeoutMS=5000, tlsAllowInvalidCertificates=True)
            else:
            ...
            print(f"Koneksi ke database MongoDB cloud 'creator_web' berhasil pada percobaan ke-{attempt}.")
            return cloud_client, creator_db, users_collection, stats_collection
        except pymongo.errors.ServerSelectionTimeoutError as e:
            print(f"Percobaan ke-{attempt}: Tidak dapat terhubung ke MongoDB cloud. (Timeout)")
            print(f"Detail error: {e}")
            # Jika error SSL, coba ulang dengan tlsAllowInvalidCertificates
            if not tried_insecure and 'certificate verify failed' in str(e):
                print("[INFO] Terdeteksi error SSL certificate, mencoba ulang dengan tlsAllowInvalidCertificates=True...")
                tried_insecure = True
                continue
            last_exception = e
        except Exception as e:
            print(f"Percobaan ke-{attempt}: Error saat membaca database: {e}")
            # Jika error SSL, coba ulang dengan tlsAllowInvalidCertificates
            if not tried_insecure and 'certificate verify failed' in str(e):
                print("[INFO] Terdeteksi error SSL certificate, mencoba ulang dengan tlsAllowInvalidCertificates=True...")
                tried_insecure = True
        ...
        if attempt < max_retries:
            ...
            time.sleep(delay)
    print("\nGagal terhubung ke MongoDB cloud setelah beberapa kali percobaan.")
    print(f"Error terakhir: {last_exception}")
    exit()

cloud_client, creator_db, users_collection, stats_collection = try_connect_mongo(max_retries=20, delay=5)
...
async def stable_sample_followers(get_value_func, page, sample_count=5, interval=0.5, timeout=30):
    """
            ...
            while True:
                ...
                try:
                    for user_info in users_to_monitor:
                            ...
                            try:
                                ...
                                while not goto_success and goto_attempt < max_goto_retries:
                                    try:
                                        ...
                                        goto_attempt += 1
                                        print(f"[GOTO-RETRY] Percobaan ke-{goto_attempt} gagal membuka URL: {url}")
                                        print(f"[GOTO-RETRY] Error: {e}")
                                        if goto_attempt < max_goto_retries:
                                            print("Menunggu 3 detik sebelum mencoba ulang page.goto...")
                        ...
                        await asyncio.sleep(0.5)  # Tambah delay antar user
                except Exception as e:
                    print(f"{RED}[EXCEPTION] {type(e).__name__}: {e}{RESET}")
                    traceback.print_exc()
                    if 'TimeoutError' in str(type(e)) or 'NetworkError' in str(type(e)):
